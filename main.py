...
from applications.ai_advisor import AIStockAdvisor, AIChatbot
...
def create_app():
    ...
    CORS(app, origins=["http://localhost:8080", "http://localhost:8081"], supports_credentials=True)
    print("Flask-CORS initialized, allowing origins: http://localhost:8080, http://localhost:8081")
    # --- End Adjustment ---
    ...
    api.add_resource(AIStockAdvisor, '/ai/analyze')  # Accessible at /api/v1/ai/analyze
    api.add_resource(AIChatbot, '/ai/chat')         # Accessible at /api/v1/ai/chat
    
    # Portfolio Management Endpoints
    api.add_resource(PortfolioHoldings, '/portfolio/holdings/<int:user_id>')  # GET all, POST new
    api.add_resource(PortfolioHoldingDetail, '/portfolio/holdings/<int:user_id>/<int:holding_id>')  # PUT, DELETE
    api.add_resource(PortfolioAnalysis, '/portfolio/analysis/<int:user_id>')  # GET analysis
    api.add_resource(PortfolioPerformance, '/portfolio/performance/<int:user_id>')  # GET performance history
    api.add_resource(PortfolioGoals, '/portfolio/goals/<int:user_id>')  # POST, GET goals
    api.add_resource(PortfolioGoalDetail, '/portfolio/goals/<int:user_id>/<int:goal_id>')  # PUT, DELETE goals
